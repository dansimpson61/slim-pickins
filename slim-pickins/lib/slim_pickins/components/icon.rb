...
module SlimPickins
  module Components
    ...
    class Icon < Component
      ...
      def render
        path = PATHS.fetch(@name.to_sym) do
          raise ArgumentError, "unknown icon #{@name.inspect}; have #{PATHS.keys.join(', ')}"
        end

        attrs = view.send(:build_attributes, @options.merge(
        ...
        safe(%(<svg#{attrs}><path stroke-linecap="round" stroke-linejoin="round" d="#{path}"></path></svg>))
      end
    end
  end
end
